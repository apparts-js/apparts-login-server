...
2. Add the package to your codebase:
   #+BEGIN_SRC js
     import { addUserRoutes, createUserModel } from "@apparts/login-server";
   ...
   - If you need to specify the route names (and methods) by yourself:
     #+BEGIN_SRC js
       ...
       const {  addUser,
                ...
                deleteUser,
                updateUser,
                resetPassword, } = routes(User, mail);
   ...
   - If you need to configure the User-model:
     - The table name (default ~users~):
       #+BEGIN_SRC js
         import { createUserModel } from "@apparts/login-server";
         const User = createUserModel(undefined, "myTableName");
       #+END_SRC
     - The types of the user (see [[https://github.com/phuhl/apparts-types#usage][@apparts/types]]):
       #+BEGIN_SRC js
         import { createUserModel } from "@apparts/login-server";
         const User = createUserModel({ customElement: { type: "string", public: true }});
       #+END_SRC
     -  Overwriting functions of the user: See [[Overwriting functions of the User-model]]
       #+BEGIN_SRC js
         import { makeModel } from "@apparts/model";
         import { createUserModel } from "@apparts/login-server";
         const Users = createUserModel(/* <custom type>, <custom table name>*/);

         // e.g. for User
         export class SpecialUser extends Users {
...
#+BEGIN_SRC js
import { createLoginsModel } from "@apparts/login-server";
const Logins = createLoginsModel();
